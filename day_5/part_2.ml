...
let add_points table seg =
    let dx = compare (seg.x2 - seg.x1) 0 in
    let dy = compare (seg.y2 - seg.y1) 0 in
    let n = 1 + max (abs (seg.x1 - seg.x2)) (abs (seg.y1 - seg.y2)) in
    let init_fun = (fun i -> (seg.x1 + dx * i, seg.y1 + dy * i)) in
...
let point_counts segments =
    ...
    in let rec aux = function
        | seg::rest -> add_points counts seg; aux rest
        | [] -> ()
